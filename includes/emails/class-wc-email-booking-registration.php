 ...
 *
 * An email sent to a custom email (authority) when a booking has been created.
 *
 * @class       WC_Email_Booking_Registration
 * @extends     WC_Email
 */
if ( ! class_exists( 'WC_Email_Booking_Registration' ) ) {
	
	class WC_Email_Booking_Registration extends WC_Email {
		...
		function __construct() {
			...
			$this->id                   = 'booking_registration';
			$this->title                = __( 'Booking Registration', 'woocommerce-bookings-authority-registration' );
			$this->description          = __( 'Booking Registration emails are sent to the authority when a new booking is created and paid.', 'woocommerce-bookings-authority-registration' );
	
			$this->heading              = __( 'Booking Registration', 'woocommerce-bookings-authority-registration' );
			$this->subject              = __( 'Registration for "{product_title}"', 'woocommerce-bookings-authority-registration' );
			$this->message              = __( 'We like to inform about the registration of {product_title}', 'woocommerce-bookings-authority-registration' );
			
			$this->message     			= $this->get_option( 'message', $this->message );
			
			$this->template_html    	= 'emails/authority-booking-registration.php';
			$this->template_plain   	= 'emails/plain/authority-booking-registration.php';
	...
	//		add_action( 'woocommerce_booking_in-cart_to_pending-confirmation_notification', array( $this, 'queue_notification' ) );
			add_action( 'woocommerce_booking_unpaid_to_paid_notification', array( $this, 'trigger' ) );
	...
	//		add_action( 'woocommerce_booking_confirmed_to_paid_notification', array( $this, 'queue_notification' ) );
			add_action( 'woocommerce_booking_confirmed_to_paid_notification', array( $this, 'trigger' ) );
	...
	//		add_action( 'woocommerce_admin_new_booking_notification', array( $this, 'trigger' ) );
		...
		public function queue_notification( $booking_id ) {
			wp_schedule_single_event( time(), 'woocommerce_booking_registration', array( 'booking_id' => $booking_id ) );
		}
		...
		function trigger( $booking_id ) {
			...
			if ( $booking_id ) {
				...
				if( $this->send( $this->get_recipient(), $this->get_subject(), $this->get_content(), $this->get_headers(), $this->get_attachments() ) ) {
					...
					// add note
					$order->add_order_note( __( 'Booking Registration has been successfully sent', 'woocommerce-bookings-authority-registration' ), false );
				
				} else {
					
					// add note
					$order->add_order_note( __( 'Booking Registration couldn\'t be sent', 'woocommerce-bookings-authority-registration' ), false );
...
return new WC_Email_Booking_Registration();
